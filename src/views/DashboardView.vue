    ...
    <div class="dashboard-layout">
      ...
      <section class="column column-right">
        <UserProfileSection
          ...
          @delete-account="handleDeleteUser"
        />
...
import { useMessageBoxStore } from '@/stores/messageBox.ts'
...
export default defineComponent({
  ...
  setup() {
    ...
    // --- Reactive Data ---
    // Basic user data is only loaded during login
    const currentUser = ref<UserFull | null>(null) // Will be populated from API
    ...
    const fetchFilterOptions = async () => {
      try {
        ...
        rankFilterOptions.value = ranksRes.data.map(
          (rank: { id: string | number; name: string }) => ({
            value: rank.id,
            label: rank.name,
          }),
        )
        ...
        languageFilterOptions.value = languagesRes.data.map(
          (lang: { id: string | number; name: string }) => ({
            value: lang.id,
            label: lang.name,
          }),
        )
    ...
    const fetchTasks = async (loadMore = false) => {
      ...
      try {
        ...
        if (activeTaskViewTab.value === 'favourites') {
          const response = await apiClient.get<PaginatedTasks>('/favourites', { params }) // Example: GET /api/tasks
          if (loadMore) {
        ...
        } else if (activeTaskViewTab.value === 'solved') {
          const response = await apiClient.get<PaginatedTasks>('/solved', { params }) // Example: GET /api/tasks
          if (loadMore) {
            displayedTasks.value.push(...response.data.items)
          } else {
            displayedTasks.value = response.data.items
    ...
    const fetchUserFull = async () => {
      try {
        const response = await apiClient.get<UserFull>('/users') // Endpoint for full user data
    ...
    const fetchUserCreatedTasks = async () => {
      ...
      try {
        const response = await apiClient.get('/tasks/user-tasks') // Example: GET /api/tasks/my-tasks
    ...
    const handleDeleteUser = async () => {
      const userConfirmed = await messageBox.confirm({
        title: 'Delete',
    ...
    const handleDeleteUserTask = async (taskToken: string | number) => {
      if (confirm('Are you sure you want to delete this task?')) {
        try {
          ...
          userCreatedTasks.value = userCreatedTasks.value.filter((task) => task.token !== taskToken)
    ...
    onMounted(async () => {
      ...
      await fetchTasks()
      await fetchUserFull() // Fetch full user data first
      await fetchUserStats() // Then fetch stats which depend on user data
...
.dashboard-view {
  ...
  min-height: calc(100vh - 20vh); /* Adjust header/footer height */
